...
class GameTexts:
    def __init__(self):
        ...
        self.photo8_repeat_texts = ["Том, что с памятью. Ты уже же все понял."]
...
class PhotoGame(arcade.Window):
    def __init__(self) -> None:
        ...
        self.photo8_visited = False  # Был ли фото8 посещен
        self.photo6_visited = False  # Был ли фото6 посещен
        self.door_closed_final_texts_completed = False  # Прокликаны ли door_closed_final_texts
    ...
    def on_draw(self) -> None:
        ...
        if self.show_final_black_screen:
            ...
            self.draw_photo8()  # Используем общий метод
    ...
    def draw_photo8(self):
        """Универсальный метод для отрисовки photo8 с текстом"""
        arcade.draw_texture_rect(self.photo_manager.photos["photo8"],
                                 arcade.XYWH(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2, SCREEN_WIDTH, SCREEN_HEIGHT))

        # Определяем, какой текст показывать
        if self.photo8_visited:
            # Если уже посещали, показываем текст повторного посещения
            if self.photo8_text_index < len(self.texts.photo8_repeat_texts):
                photo8_text = self.texts.photo8_repeat_texts[self.photo8_text_index]
    ...
    def draw_report(self):
        ...
        # Если уже посещали photo6, показываем все тексты сразу
        if self.photo6_visited:
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if self.door_closed_final_active:
            if self.door_closed_final_index < len(self.texts.door_closed_final_texts) - 1:
                ...
                self.door_closed_final_texts_completed = True  # Отмечаем, что тексты прокликаны
                self.show_goal = True
                self.goal_text = "Беги"
            return
    ...
    def handle_photo2_click(self, x, y):
        if (self.current_state == "photo2" and
            ...
            self.show_goal = False
            return
        # ИЗМЕНЕНИЕ: Проверяем, прокликаны ли door_closed_final_texts
        if (self.current_state == "photo2" and
                self.final_texts_completed and
                self.goal_text == "Беги" and
                self.click_areas.is_in_area(x, y, "door_photo2")):
            # Позволяем переходить только если тексты door_closed_final_texts уже прокликаны
            if self.door_closed_final_texts_completed:
        ...
        if (self.final_texts_completed and
                self.goal_text == "Беги" and
                self.click_areas.is_in_area(x, y, "back_to_photo1")):
            # Если тексты door_closed_final_texts еще не прокликаны, показываем их
            if not self.door_closed_final_texts_completed:
    ...
    def handle_report_click(self, x, y):
        if self.click_areas.is_in_area(x, y, "back_from_photo6"):
            ...
            # Отмечаем, что photo6 был посещен
            if not self.photo6_visited:
                self.photo6_visited = True
        ...
        # Если photo6 уже посещали, просто выходим при клике
        if self.photo6_visited:
            self.current_photo = self.photo_manager.photos["photo3"]
            self.current_state = "photo3"
            self.came_from_photo6 = True
        ...
        if len(self.current_report_texts) < len(self.texts.report_texts):
            ...
            if len(self.current_report_texts) == len(self.texts.report_texts):
                ...
                # Отмечаем, что photo6 был посещен после завершения отчета
                self.photo6_visited = True
    ...
    def handle_photo8_click(self, x, y):
        ...
        # Определяем, какой текст показывать
        if self.photo8_visited:
            # Если уже посещали, используем повторный текст
            if self.photo8_text_index < len(self.texts.photo8_repeat_texts) - 1:
            ...
            # Если первый раз, используем обычный текст
            if self.photo8_text_index < len(self.texts.photo8_texts) - 1:
                self.photo8_text_index += 1
            else:
                # После завершения обычного текста отмечаем, что photo8 был посещен
                self.photo8_visited = True
    ...
    def handle_photo9_click(self, x, y):
        # БЛОКИРОВКА ПЕРЕХОДА НА PHOTO8, ЕСЛИ ЦЕЛЬ "Найти инфу про дверь"
        if (self.goal_text == "Найти инфу про дверь" and
                self.click_areas.is_in_area(x, y, "photo9_to_photo8")):
            # Не разрешаем переход на photo8
            return
